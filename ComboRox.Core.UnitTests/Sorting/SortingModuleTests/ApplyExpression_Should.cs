...
namespace ComboRox.Core.UnitTests.Sorting.SortingExpressionFactoryTests
{
    ...
    public class ApplyExpression_Should
    {
        ...
        [TestMethod]
        public void ReturnSortedCollection_WhenThereAreTwoAscCriterias()
        {
            // Arrange
            var listOfItems = new List<UserTestingClass>
                {
            ...
            var modulesSettings = new ModulesSettings
            {
                Sorting =
                    new List<SortingObject>
                        {
                            new SortingObject { Prop = "Age", Direction = SortingDirection.Asc },
                            new SortingObject { Prop = "FirstName", Direction = SortingDirection.Asc }
                        }
            };
            ...
            Assert.IsTrue(sortedCollection.Any());
            Assert.AreEqual(5, sortedCollection.First().Age);
            Assert.AreEqual("Connie", sortedCollection.First().FirstName);

            Assert.AreEqual(10, sortedCollection[3].Age);
            Assert.AreEqual("Eadgar", sortedCollection[3].FirstName);
        ...
        [TestMethod]
        public void ReturnSortedCollection_WhenThereAreTwoDescCriterias()
        {
            // Arrange
            var listOfItems = new List<UserTestingClass>
                {
            ...
            var modulesSettings = new ModulesSettings
            {
                Sorting =
                    new List<SortingObject>
                        {
                            new SortingObject { Prop = "Age", Direction = SortingDirection.Desc },
                            new SortingObject { Prop = "FirstName", Direction = SortingDirection.Desc }
                        }
            };
            ...
            // Assert
            Assert.IsTrue(sortedCollection.Any());
            Assert.AreEqual(33, sortedCollection.First().Age);
            Assert.AreEqual("Barny", sortedCollection.First().FirstName);

            Assert.AreEqual(5, sortedCollection[3].Age);
            Assert.AreEqual("Danny", sortedCollection[3].FirstName);

            Assert.AreEqual(5, sortedCollection.Last().Age);
            Assert.AreEqual("Connie", sortedCollection.Last().FirstName);
        }
    }
}